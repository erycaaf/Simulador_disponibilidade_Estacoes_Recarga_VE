...
try:
    ...
    c_lib.calculate_charging_time.argtypes = [ctypes.c_float, ctypes.c_float, ctypes.c_float]
...
def estimate_time(battery_kwh: float, current_percent: float, power_kw: float) -> float:
    """
    ...
    result = c_lib.calculate_charging_time(battery_kwh, current_percent, power_kw)
    return result
